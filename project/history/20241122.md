...
✔  Firebase initialization complete!
nameMacBook-Pro project % firebase logout
✔  Logged out from email@gmail.com
nameMacBook-Pro project % firebase login
...
https://accounts.google.com/o/oauth2/auth?client_id=563584335869-fgrhgmd47bqnekij5i8b5pr03ho849e6.apps.googleusercontent.com&scope=email%20openid%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcloudplatformprojects.readonly%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Ffirebase%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcloud-platform&response_type=code&state=981838504&redirect_uri=http%3A%2F%2Flocalhost%3A9005

Waiting for authentication...

✔  Success! Logged in as email@gmail.com
nameMacBook-Pro project % 
```
https://gyazo.com/30dcf6b6c76d7056f6d87df07d18d7d3

```
nameMacBook-Pro project % firebase projects:list
...
│ *************          │ *************       │ *************     │ [Not specified]      │
├──────────────────────┼──────────────────────────┼────────────────┼──────────────────────┤
│ tec-support-pj       │ tec-support-pj (current) │ 651034449412   │ [Not specified]      │
└──────────────────────┴──────────────────────────┴────────────────┴──────────────────────┘

2 project(s) total.
nameMacBook-Pro project % firebase deploy --only hosting
...
+ First Load JS shared by all            81.9 kB
  ...
  ├ chunks/fd9d1056-891a55ec38545216.js  53.3 kB
  ├ chunks/main-app-c5f850f29b4c46b3.js  220 B
  └ chunks/webpack-d86cdceb49919631.js   1.73 kB
...
│ Project Display Name │ Project ID               │ Project Number │ Resource Location ID │
├──────────────────────┼──────────────────────────┼────────────────┼──────────────────────┤
│ study-record         │ study-record-930dc       │ 31468894948    │ [Not specified]      │
├──────────────────────┼──────────────────────────┼────────────────┼──────────────────────┤
│ tec-support-pj       │ tec-support-pj (current) │ 651034449412   │ [Not specified]      │
└──────────────────────┴──────────────────────────┴────────────────┴──────────────────────┘
...
Visit this URL on this device to log in:
https://accounts.google.com/o/oauth2/auth?client_id=563584335869-fgrhgmd47bqnekij5i8b5pr03ho849e6.apps.googleusercontent.com&scope=email%20openid%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcloudplatformprojects.readonly%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Ffirebase%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcloud-platform&response_type=code&state=906919161&redirect_uri=http%3A%2F%2Flocalhost%3A9005
...
32 packages are looking for funding
  run `npm fund` for details
i  hosting: The following function(s) are pinned to site tec-support-pj and will be deployed as well: ssrtecsupportpj

=== Deploying to 'tec-support-pj'...

i  deploying functions, hosting
i  functions: preparing codebase firebase-frameworks-tec-support-pj for deployment
...
Error: Your project tec-support-pj must be on the Blaze (pay-as-you-go) plan to complete this command. Required API artifactregistry.googleapis.com can't be enabled until the upgrade is complete. To upgrade, visit the following URL:

https://console.firebase.google.com/project/tec-support-pj/usage/details
...
エラーメッセージから判断すると、FirebaseプロジェクトがBlaze（従量課金）プランにアップグレードされていないため、必要なAPIを有効化できず、デプロイが失敗していることが原因です。
...
```
## src/middleware.tsx を削除すれば、Cloud Functions が不要になるため、Spark プラン（無料プラン）でもデプロイが可能になります。
...
i  deploying hosting
i  hosting[tec-support-pj]: beginning deploy...
i  hosting[tec-support-pj]: found 26 files in .firebase/tec-support-pj/hosting
✔  hosting[tec-support-pj]: file upload complete
i  hosting[tec-support-pj]: finalizing version...
✔  hosting[tec-support-pj]: version finalized
i  hosting[tec-support-pj]: releasing new version...
✔  hosting[tec-support-pj]: release complete

✔  Deploy complete!

Project Console: https://console.firebase.google.com/project/tec-support-pj/overview
Hosting URL: https://tec-support-pj.web.app
nameMacBook-Pro project % 
```
## デプロイに成功する
[ドメイン](tec-support-pj.web.app)
[デフォルト](tec-support-pj.firebaseapp.com)

# ToDo
- [x] 期限日の入力バリデーションを追加
~~- [ ] 「更新日」が、時間も反映されるようにする~~
- [x]  「追加」ボタンを押した後、一度画面をリロードしないと、ToDoの数が更新されない(増えない)部分を修正する
...
+ First Load JS shared by all            81.9 kB
  ...
  ├ chunks/fd9d1056-891a55ec38545216.js  53.3 kB
  ├ chunks/main-app-c5f850f29b4c46b3.js  220 B
  └ chunks/webpack-e39ed28961f3d065.js   1.73 kB
...
/Users/name_1/Desktop/_workspace_tec/workspace_React-todo_1/React-todo_1/project/functions/src/index.ts
  10:9   warning  'onRequest' is defined but never used  @typescript-eslint/no-unused-vars
  11:13  warning  'logger' is defined but never used     @typescript-eslint/no-unused-vars
...
src/index.ts ファイルにおいて、以下の2つのインポートが宣言されているものの、使用されていません：

typescriptCopyimport {onRequest} from "firebase-functions/v2/https";
import * as logger from "firebase-functions/logger";

なお、このエラーは firebase deploy コマンド実行時の関数のデプロイ前チェック（predeploy）で検出されています。TypeScriptの厳格な型チェックにより、未使用のインポートはエラーとして扱われ、デプロイがブロックされています。

コード内で onRequest や logger を実装してください。

## 機能の意味
- プログラムの中で`onRequest`を使って、ウェブサイトへのアクセスに対応する処理を書く
- `logger`を使って、プログラムの動きを記録する
...
エラー： このコマンドを実行するには、プロジェクト tec-support-pj が Blaze（従量課金）プランである必要があります。必須 API artifactregistry.googleapis.com は、アップグレードが完了するまで有効にできません。


## 無料プラン（Spark プラン）でFirebaseを利用する
```
nameMacBook-Pro project % npm run build
...
Route (app)                              Size     First Load JS
┌ ○ /                                    36.5 kB         118 kB
└ ○ /_not-found                          875 B          82.9 kB
+ First Load JS shared by all            82 kB
  ├ chunks/938-8342a39d4fe24c47.js       26.8 kB
  ├ chunks/fd9d1056-891a55ec38545216.js  53.3 kB
  ├ chunks/main-app-c5f850f29b4c46b3.js  220 B
  └ chunks/webpack-a1de3d416f26c733.js   1.73 kB
